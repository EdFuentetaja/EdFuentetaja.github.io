...
It's time now to take another card from under our sleeve, this time is the "narrow-band assumption." We claim that our m(t) signal is changing slowly, at least compared to the speed of change of our carrier frequency $$f_c$$. To see how this is fair, let's give some numbers, let's say that we make R equal to one quarter of our wavelength $$\lambda$$, and let's say we are considering WiFi signals at 2.4 GHz, then our R is about 3.125 cm, which means a worst case time difference of about 0.1 nanoseconds to our radio signal. Considering that our WiFi signal has a bandwidth of 40 MHz, the highest frequency it's going to carry is 20 MHz, which has a period of 50 ns. The compromise here is to say that a sinusoid with period 50 ns is not going to change much from time t to time t+0.1 ns. In fact we can say that the maximum change we are going to see with this delay in this example is about 1% ($$sin(2 \pi 0.1/50) \approx 0.0125$$).
